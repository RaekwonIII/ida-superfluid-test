...
async function main() {
    ...
    let shareGainer = alice // SELECT FROM ALICE, BOB, CAROL, OR MALLORY at your discretion
    ...
    const sETHx = await sf.loadSuperToken(`${process.env.sETHx}`)
    ...
    try {
        ...
        const subscribeOperation = sETHx.approveSubscription({
            indexId: await tokenSpreader.INDEX_ID(),
            publisher: tokenSpreader.address
        })
